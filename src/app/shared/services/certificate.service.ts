...
export class CertificateService {
  ...
  constructor(
    ...
    this._store
      .select(fromRoot.getCurrentCertificate)
      ...
      content: [
        {
          columns: [
            {
              qr:
                "GEMENI ENTERPRISES GOA\nSLD_NUMBER: " +
                certificate.device.sld_number +
                "\nCERTIFICATE NUMBER: " +
                certificate.certificate_number +
                "\nDATE GENERATED: " +
                this._datePipe.transform(
                  certificate.date_generated,
                  "yyyy-MM-dd"
                ) +
                "\nDUE DATE: " +
                this._datePipe.transform(certificate.due_date, "yyyy-MM-dd") +
                "\nCHASSIS NUMBER: " +
          ...
          columns: [
            {
              text: [
                ...
                {
                  text:
                    certificate.vehicle.make +
                    ", " +
                    certificate.vehicle.model +
                    " " +
                    certificate.vehicle.variant,
                  bold: true,
                },
                "\n\nDate of check: ",
                {
  ...
  generateGeneralContent(certificate: Certificate, type: string): any[] {
    return [
        ...
        columns: [
          [
            ...
            },
            {
              text:
                "The Regional\n" +
                certificate.location_rto +
                ", " +
                certificate.location_state,
            },
          ],
          [
            {
            ...
            {
              text:
                this.certificate.renewal_count === 0
                  ? "Fitment Assurance"
      ...
      {
        text:
          "The above mentioned fitment complies with ISO standard 9001:2015 and is tamper proof to the best of the certifying " +
          "authority's knowledge. Our fitments comply with ICAT norms AIS-018. The manufacturer is not responsible for any misuse " +
          "or malpractice towards the" +
          (this.certificate.renewal_count === 0 ? "" : " renewal") +
      ...
      },
      {
        ol: [
          "This is to acknowledge and confirm that we have got our vehicle bearing above Registration No.: " +
            ...
            (this.certificate.renewal_count === 0
              ? ""
              : " and same has been re-calibrated."),
          "After " +
            (this.certificate.renewal_count === 0
              ? "fitting"
              : "re-calibrating") +
  ...
  printIcat(certificate: Certificate) {
    ...
    certificate.vehicle.icats.map((icat, indexIcat) => {
      icat.pages.map((page) => {
        const base64 = this.icats_global.find(
          (data) => data.url === page.getPageUrl()
        );
        const x: any = {
          image: base64.data,
          fit: [595, 842],
  ...
  certificateActions(type: boolean) {
    ...
    if (this.certificate.vehicle.icats.length === 0) {
      ...
      type
        ? pdfMake
            .createPdf(this.certificateDoc)
            .download(this.certificate.certificate_number + ".pdf")
      ...
      if (this.certificate.location_state === "Delhi") {
        this.getBase64ImageFromURL(
          this.certificate.picture_data.getPicUrl()
        ).subscribe((data) => {
          ...
          if (data) {
            ...
            this.certificate.vehicle.icats.map((icat) => {
              icat.pages.map((page) => {
                this.getBase64ImageFromURL(page.getPageUrl()).subscribe(
                  (cont) => {
                    toast({
                      title:
                        "Downloading " +
