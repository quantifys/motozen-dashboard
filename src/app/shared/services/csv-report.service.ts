...
export class CsvReportService {
  ...
  subscribeToCertificateReport() {
    this.certificateSubscription = this._store
      .select(fromRoot.getReportCertificates)
  ...
  subscribeToPOSummary() {
    this.poSummarySubscription = this._store
      .select(fromRoot.getPOSummary)
      .subscribe((summary) => {
        if (summary.length > 0) {
  ...
  subscribeToPODetails() {
    this.poDetailsSubscription = this._store
      .select(fromRoot.getPODetails)
      .subscribe((details) => {
        if (details.length > 0) {
  ...
  subscribeToStockSummary() {
    this.stockSummarySubscription = this._store
      .select(fromRoot.getStockSummary)
      .subscribe((summary) => {
        if (summary.length > 0) {
  ...
  generatePODetails(details: any[]) {
    ...
    details.map((detail, index) => {
      let data: any = {
        ...
        vehicles: detail.particulars.map(
          (particular) =>
            particular.vehicle.make +
            " " +
            particular.vehicle.model +
            " " +
            particular.vehicle.variant +
            "(" +
            particular.quantity +
            ")"
        ),
    ...
    new Angular5Csv(
      csvData,
      "po-list-report " + new Date().toDateString(),
      options
    );
  }
  ...
  generateCertificateCsv() {
    ...
    new Angular5Csv(
      csvData,
      "certificate-report " + new Date().toDateString(),
      options
    );
  }
  ...
  generatePOSummary(summary: any, type: boolean) {
    ...
    new Angular5Csv(
      summary.filter((data, index) => index > 0),
      type ? "po-summary " : "stock-summary" + new Date().toDateString(),
      options
    );
  }
}
