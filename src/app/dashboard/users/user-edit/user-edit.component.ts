...
export class UserEditComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this._store.select(fromRoot.getAllUsers).subscribe(distributors => {
  ...
  fetchUsers(type: string) {
    this.loading = true;
    this._store.dispatch(new userActions.FetchAllUsersAction({ role: type }));
  }

  filterRoles() {
    if (this.loggedUser.role === 'human_resource') {
      this.roles = this.roles.filter(role => (
        role.value !== 'dealer' && role.value !== 'sub_dealer'
        && role.value !== 'distributor' && role.value !== 'admin' && role.value !== 'manufacturer'));
  ...
  formListener() {
    ...
    this.distributor_id.valueChanges.subscribe(value => {
      if (this.role.value === 'dealer' || this.role.value === 'sub_dealer') {
        const distributor: User = this.distributors.find(dist => dist.id === this.distributor_id.value);
        if (distributor) {
  ...
  addressValidate() {
    if (this.addUser) {
      const controls: string[] = ['address_l1', 'address_l2', 'locality', 'city', 'state', 'pincode'];
    ...
    const employeeControls: string[] = ['base_salary', 'hra', 'transport_allowance', 'gpf', 'esic'];
    if (this.role.value === 'rto') {
      detailControls = detailControls.filter(data => data !== 'state' && data !== 'state_code');
    }
    detailControls.map(control => {
      if (this.role.value !== 'distributor' && this.role.value !== 'dealer' && this.role.value !== 'sub_dealer') {
    ...
    employeeControls.map(control => {
      if (this.role.value !== 'distributor' && this.role.value !== 'dealer'
        && this.role.value !== 'sub_dealer' && this.role.value !== 'rto') {
        this.details.get(control).enable();
      } else {
        this.details.get(control).disable();
      }
  ...
  saveChanges() {
    if (this.addUser) {
      ...
      if (formData['role'] === 'distributor' || formData['role'] === 'dealer' || formData['role'] === 'sub_dealer') {
        ...
        if (formData['role'] === 'sub_dealer') {
          formData['dealer_id'] = formData['distributor_id'];
