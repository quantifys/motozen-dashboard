...
export class PurchaseOrdersComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.userSubscription$ = this._store.select(fromRoot.getLoggedUser).subscribe(user => {
      ...
      if (user.role) {
        if (user.role === 'manufacturer' || user.role === 'distributor' || user.role === 'store_purchases' || user.role === 'admin'
          || user.role === 'accounts' || user.role === 'store_dispatch' || user.role === 'store_logistics' || user.role === 'sales') {
          const newParams: any = {};
  ...
  detailsReport() {
    const data: any = this._activatedRoute.snapshot.queryParams;
